<template>
    <section class="detail-wrap">
        ...
        <div class="canvas-bg" v-show="bg">
            <div></div>
            <canvas canvas-id="shareCanvas"></canvas>
            <btn  cname="big-btns" text="下载图片"></btn>
        </div>
         
        
        <ul class="detail-swiper-tab nav-first">
            <li :class="[swiperTabList,currentTab==0?'on':'']" data-current="0" >
                <span>活动</span>
            </li>
            <li :class="[swiperTabList,currentTab==1?'on':'']" data-current="1">
                <span>详情</span>
            </li>
        </ul>
        <div class="detail-content-box">
            <div class="detail-content">
                <topSlider enabled="3" activeColor="#f85354" url="url" indicatorColor="#fff" :movies="discountsImage"></topSlider>
                <span class="detail-goods-bg">
                    <span class="detail-left">
                        ...
                        <span class="target-detail">
                            <span class="detail-collectioned">
                                已参与：{{shopping_num?shopping_num:0}}人
                            </span>
                ...
                <span class="detail-info detail-info2" style="padding:0;z-index:99;">
                    <button :hoverStopPropagation="true" class="detail-share">
                            分享
                    </button>
                </span>
                <span class="detail-info detail-info2" style="padding:0;z-index:99;">
                ...
                <span class="detail-dec">
                    ...
                    <dl v-if="genre==2"  class="discount-list">
                        ...
                        <dt :class="level>=2?'bg':''" v-if="three">
                    ...
                    <!-- <span class="enroll" v-if="joinList">
                        已报名(33)
                        <span >更多</span>
                    </span>
                    <ul class="enroll-list" v-if="joinList">
                        <li v-for="(item,index) in joinList" :key="index">
                            <img :src="item.avatar" alt="">
                            <span>{{item.nickname}}</span>
                        </li>
                    </ul> -->
                    <span v-if="detailShow" class="important-tip">
                        <img src="../../../static/img/icon/help.png" alt=""> 活动详情
                    </span>
                    <span class="center-text" v-if="!detailShow">
                        下拉显示详情
                    </span>
                    <span v-show="detailShow" class="item-desc">{{item_desc}}</span>
    ...
    methods: {
        ...
        getDetail(id){
            ...
            util.request(api.detail,{
                ...
                that.item_desc = res.data.desc[0].item_desc;
                ...
                if(that.shopping_num>that.five&&that.five!=0){
                    that.level = 3;
                }else if(that.shopping_num>that.three&&that.three!=0){
                    that.level = 2;
                }else if(that.shopping_num>that.one){
                    that.level = 1;
                }else{
                    that.level = 0;
                }
        ...
        getJoin(id){
            ...
            util.request(api.queryJoin,{
                ...
                that.joinList = res.data.userList
            });
        },
    },
    onShow(){
