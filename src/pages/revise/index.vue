<template>
    <section @click="bodyClick" class="act-wrap">
        <dl class="act-edit act-edit2">
            <dt>
                <span>活动名称</span>
                <input v-model="dataList.title" placeholder="请输入活动名称，如海尔空调优惠活动" />
            </dt>
            <dt>
                <span >行业分类</span>
                <span style="width:130px"  @click="showPicker" class="all">{{valueClassfy}}</span>
            ...
            <dt>
                <span>市场售价</span>
                <input v-model="dataList.zero" type="number" placeholder="请输入市场售价" />
            </dt>
            <dt>
                <span>最低起购数</span>
                <input v-model="dataList.least" type="number" placeholder="默认为1，指单次参加活动的最低起购数量" />
            </dt>
            <dt>
                <span>库存数量</span>
                <input v-model="dataList.num" type="number" placeholder=请输入参加活动的商品数量或名额 />
            </dt>
            <dt>
                <span>限购数</span>
                <input v-model="dataList.most" type="number" placeholder="默认不限购，指对单个用户限购数量" />
            </dt>
            <dt>
                <span>活动亮点</span>
                <input v-model="dataList.sell_point" type="text" placeholder="简单描述下活动的亮点" />
            </dt>
            <dt>
                <span>到店核销码</span>
                <input v-model="dataList.check" maxlength="8" type="number" placeholder=设置身份识别的8位数字作为密码 />
            </dt>
            <dt>
        ...
        <dl class="act-edit">
            ...
            <dt style="line-height:14px;margin-top:10px;">
                <span>商品或活动描述（显示在详情页详情中）</span>
                <textarea maxlength="500" class="textarea-input" v-model="dataList.item_desc" placeholder="请输入商品的描述，或者活动的具体内容，也可对活动那个规则进行补充说明。" name="textarea"/>
                <div style="margin-bottom:10px;">
                    <ul>
                        <span style="display:inline-block;width:100%;margin-bottom:10px;">活动详情（最多可添加5张图片）</span>
                        <span>
        ...
        <dl class="act-edit">
            <dt class="edit-title">
                <span style="width:auto" class="act-edit-title">优惠设置</span>
                <span @click="editClick(1)" :class="editCheck==1?'edit-checked edit-check':'edit-check'"><span></span></span><span>限时抢购</span><span @click="editClick(2)" :class="editCheck==2?'edit-checked edit-check':'edit-check'"><span></span></span><span>拼团活动</span><span @click="editClick(3)" :class="editCheck==3?'edit-checked edit-check':'edit-check'"><span></span></span><span>免费活动</span>
            </dt>
            <div v-if="editCheck==1" class="posi-rela" style="margin-bottom:10px;">
                <dt class="small">
                    <span>优惠价格</span>
                    <input v-model="favP" type="number" placeholder="请输入优惠后的价格" />
                </dt>
            </div>
            <dt v-if="editCheck==1" style="text-align:center;height:auto">
                <p>
                    <img width="13" height="13"  src="../../../static/img/icon/help.png" /><span>注:</span>限时请购，用户在线报名参加活动后，在活动有限期间，随时可到活动发起方的指定活动地点，享受活动优惠，参加活动
                </p>
            </dt>
            ...
            <div v-if="editCheck==2" class="posi-rela" v-for="(item,index) in proList" :key="index">
                ...
                <dt class="normal2">
                    <span>参加人数</span>
                    <input v-model="item.joinNun" type="number" placeholder="请输入活动成功的标志" />
                </dt>
                <dt class="small2">
                    <span>优惠价格</span>
                    <input v-model="item.joinAct" type="number" placeholder="请输入满足目标人数后的优惠价格" />
                </dt>
                <dt v-if="proList.length>1" style="text-align:center;height:auto;margin-top:10px;">
                    <div @click="clickDelPro(index)" class="add-btn-project add-btn-project2">
                        <img width="13" height="12.5" src="../../../static/img/icon/add2.png" />
                        减少优惠项目
                    </div>
                </dt>
            </div>
            <dt v-if="editCheck==2" style="text-align:center;height:auto">
                <a @click="showTip=true" id="show" style="display:inline-block;text-decoration-line:underline;color:#458165;width:100%;text-align:center;font-size:14px;">查看填写例子</a>
        ...
        <dl class="act-edit">
            <dt>
                <span class="act-edit-title">地址选择<span @click="siteClick(1)" :class="siteCheck==1?'edit-checked edit-check':'edit-check'"><span></span></span><span>签约注册时地址</span></span>
                <div style="line-height:15px;">
                    <span style="margin-left:0" @click="siteClick(2)" :class="siteCheck==2?'edit-checked edit-check':'edit-check'"><span></span></span><span>其他地址</span>
                    <span v-if="siteCheck!=2" class="heigth26" style="margin-left:10px;">{{dataList.item_site}}</span>
                    <input v-if="siteCheck==2" v-model="dataList.item_site" type="text" placeholder="请填写地址信息" />
                </div>
            </dt>
            <dt style="line-height:14px;">
                <span style="margin-top:15px;margin-bottom:14px;" class="act-edit-title">关于发货（可多选）</span> 
                <span>发货方式</span>
                <span></span><span  @click="change1(1)" :class="checkbox1?'edit-checked edit-check':'edit-check'"><span></span></span><span>自提/到店消费</span><span  @click="change2(1)" :class="checkbox2?'edit-checked edit-check':'edit-check'"><span></span></span><span>送货上门</span>
                <textarea maxlength="500" class="textarea-input" v-model="dataList.supplement" placeholder="发货方式补充说明，如有则填写，比如5公里内免运费，超距的运费计算方式等，也可以线下协商确定，非必填" name="textarea"/>
                <span style="margin-left:0px;" @click="agreeClick(1)" :class="agreeCheck==1?'edit-checked edit-check':'edit-check'"><span></span></span><span>我已阅读并遵守<a style="display:inline-block;text-decoration-line:underline;color:#458165">《活动呗发布须知和规则》</a></span>
                <span class="reg-btn">
                    <btn @handle-click="submit" cname="big-btn" text="保存"></btn>
                </span>
            </dt>
        </dl>
...
data() {
    ...
    methods: {
        ...
        getDetail(config){
            let that = this;
            util.request(api.preview,{
                itemId:config.id
            }).then(res => {
                that.dataList.title = res.data.item[0].title;
                that.dataList.itemId = config.id;
                that.dataList.discounts_id = res.data.item[0].discounts_id;
                that.dataList.zero = res.data.discounts[0].zero;
                that.dataList.least = res.data.discounts[0].least;
                that.dataList.num = res.data.item[0].num;
                that.dataList.most = res.data.discounts[0].most;
                that.dataList.sell_point = res.data.item[0].sell_point;
                that.dataList.check = res.data.item[0].check;
                that.start_time = that.formatDate(res.data.item[0].start_time);
                that.check_time = that.formatDate(res.data.item[0].check_time);
                that.dataList.item_desc = res.data.desc[0].item_desc;
                that.dataList.supplement = res.data.item[0].supplement?res.data.item[0].supplement:'';
                that.editCheck = res.data.item[0].genre;
                that.subdivide = res.data.item[0].item_cat_id;
                that.dataList.item_site = res.data.item[0].item_site;
                that.phtotList = JSON.parse(res.data.item[0].image);
                that.phtotLists = JSON.parse(res.data.desc[0].item_img);
                if(that.editCheck==1){
                    that.favP = res.data.discounts[0].one;
                }else if(that.editCheck==2){
                    that.proList[0].joinAct=res.data.discounts[0].one;
                    that.proList[0].joinNun=res.data.discounts[0].two;
                    if(res.data.discounts[0].three||res.data.discounts[0].three===0){
                        let t = {};
                        t.joinAct = res.data.discounts[0].three;
                        t.joinNun = res.data.discounts[0].four;
                        that.proList.push(t);
                    }
                    if(res.data.discounts[0].five||res.data.discounts[0].five===0){
                        let t = {};
                        t.joinAct = res.data.discounts[0].five;
                        t.joinNun = res.data.discounts[0].six;
                        that.proList.push(t);
                    }
                }
                if(res.data.item[0].goods_desc.indexOf('自提')>=0){
                    that.checkbox1 = true;
                }else{
                    that.checkbox1 = false;
                }
                if(res.data.item[0].goods_desc.indexOf('送货')>=0){
                    that.checkbox2 = true;
                }else{
                    that.checkbox2 = false;
                }
                this.getClassify();
            });
        },
        ...
        submit(){
            ...
            if(this.editCheck==2&&this.proList.length>0){
                this.proList.map((item,index)=>{
                    ...
                    if(item.joinNun>this.dataList.num){
                        wx.showModal({
                            showCancel:false,
                            title:'提示',
            ...
            this.dataList.check = this.dataList.check+'';
           ...
           /*  if(!this.dataList.one&&this.editCheck==2){
            ...
            util.request(api.update,
                this.dataList
            ).then(res => {
                wx.redirectTo({  url:'../me/main?id=1' })
                wx.hideLoading();
                console.log(res);
            })
        },
        ...
        getChildren(id,ix,name){
            ...
            that.array.map(item=>{
                if(item.parent_id==pID){
                    ...
                    data.push(obj);
                    if( item.item_cat_id==that.subdivide){
                        that.valueClassfy =  name+'-'+obj.label;
                    }
                }
            })
            return data
        ...
        getClassify(){
            ...
            util.request(api.classify).then(res => {
                ...
                res.data.catList.map((item,index)=>{
                    if(item.is_parent==1){
                        let obj = {};
                        obj.label = item.name;
                        obj.value = item.item_cat_id;
                        obj.children = that.getChildren(item.item_cat_id,that.multiArray.length,obj.label);
